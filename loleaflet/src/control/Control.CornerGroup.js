...
/*	Corner header deserves its own file, though it has only one feature:)
	Calc only.
	Row header's "start y" depends on Column header's height, column header's "start x" depends on Row header's width.
	This section gets their information and re-sizes itself. Then the others can expand accordingly.
*/

/* global $ _UNO */
L.Control.CornerHeader = L.Control.Header.extend({
	name: 'corner header',
	anchor: ['top', 'left'],
	position: [0, 0],
	size: [48 * window.devicePixelRatio, 19 * window.devicePixelRatio],
	expand: [''], // Don't expand.
